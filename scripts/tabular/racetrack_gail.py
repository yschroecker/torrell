...
from scripts.tabular import supervised
...
def _run():
    ...
    for _ in tqdm.trange(num_demo_episodes):
        is_terminal = False
        state = env.reset()
    ...
    for iteration in trange:
        ...
        for t in range(500):
            ...
            if iteration > 1:
                ...
                current_policy.parameters += 0.1 * (
                    current_policy.log_gradient([state], [action]) * (V[next_state] + reward - V[state])
                )
                # if t == 100:
