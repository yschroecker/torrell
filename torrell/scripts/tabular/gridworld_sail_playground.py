...
import torrell.scripts.tabular.policy
...
def estimate_gradient0(X, y, env: environments.tabular.TabularEnv, policy: torrell.scripts.tabular.policy.TabularPolicy) -> \
        np.ndarray:
    return policy.log_gradient(X, y)


def estimate_gradient1(X, y, env: environments.tabular.TabularEnv, policy: torrell.scripts.tabular.policy.TabularPolicy) -> \
        np.ndarray:
...
def estimate_gradient2(X, y, env: environments.tabular.TabularEnv, policy: torrell.scripts.tabular.policy.TabularPolicy) -> \
        np.ndarray:
...
def _run():
    ...
    for estimate_gradient in methods:
        policy = torrell.scripts.tabular.policy.TabularPolicy(grid.num_states, grid.num_actions, biased=False)
